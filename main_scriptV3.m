...
global gyms;
global isOnLine
global isSegmentLogicActive;
isSegmentLogicActive=false;% controlled by shift+i
...
%if(isChangedForSave==true)
    ...
    if(size(event.Modifier,2)>0)
        ...
        elseif(strcmp(event.Key,'l') && strcmp(event.Modifier{1,1},'shift'))
            ...
            run(shops,gyms);
            title('press shift+s to save and shift+l to load')
        elseif(strcmp(event.Key,'i') && strcmp(event.Modifier{1,1},'shift'))
            title('RUNNING ...')
            isSegmentLogicActive=true;
            run(shops,gyms);
        end
    end
%end
...
for i=1:size(shopsPOIs,2)
    bringToFront(shopsPOIs{1,i});
end
for i=1:size(gymsPOIs,2)
    bringToFront(gymsPOIs{1,i});
end
%disp('!!!')
end
...
a = gca;
a.Position(3) = 0.6;
helpString="press shift+s to save and shift+l to load."+newline;
onOffString='';
if isSegmentLogicActive==true
    onOffString="ON segement logic (shift+i)";
    helpString=helpString + onOffString;
else
    onOffString="OFF segement logic (shift+i)";
    helpString=helpString + onOffString;
end
annotation('textbox', [0.75, 0.1, 0.24, 0.8], 'String',helpString)
end